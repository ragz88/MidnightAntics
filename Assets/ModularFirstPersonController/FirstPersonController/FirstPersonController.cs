...
public class FirstPersonController : MonoBehaviour
{
    ...
    public float xLookSensitivity = 2f;
    public float yLookSensitivity = 2f;
    ...
    private void Update()
    {
        if (walkingMode)
        {
            ...
            if (cameraCanMove)
            {
                ...
                if (!invertCamera)
                {
                    pitch -= yLookSensitivity * Input.GetAxis("LookY");
                }
                else
                {
                    // Inverted Y
            ...
            // Looks for Sprint Input, considering whether sprint is a hold or a toggle.
            if (holdToSprint)
            {
                sprintInput = Input.GetButton(sprintButton);
            }
            else
            {
            ...
            if (enableCrouch)
            {
                if (Input.GetButtonDown(crouchButton) && !holdToCrouch)
                {
                    Crouch();
                }
                ...
                else if (Input.GetButtonUp(crouchButton) && holdToCrouch)
                {
                    isCrouched = true;
                    Crouch();
                }
        ...
        else
        {
            ...
            if (!invertCamera)
            {
                pitch -= yLookSensitivity * Input.GetAxis("Pitch");
            }
            else
            {
                // Inverted Y
...
// Custom Editor
    ...
    public override void OnInspectorGUI()
    {
        ...
        fpc.xLookSensitivity = EditorGUILayout.Slider(new GUIContent("X Look Sensitivity", "Determines how sensitive the Horizontal mouse movement is."), fpc.xLookSensitivity, .1f, 10f);
        fpc.yLookSensitivity = EditorGUILayout.Slider(new GUIContent("Y Look Sensitivity", "Determines how sensitive the Vertical mouse movement is."), fpc.yLookSensitivity, .1f, 10f);
        ...
        fpc.holdToSprint = EditorGUILayout.ToggleLeft(new GUIContent("Hold To Sprint", "Determines if sprint is a toggle or a hold."), fpc.holdToSprint);
        fpc.sprintButton = EditorGUILayout.TextField(new GUIContent("Sprint Button", "Determines what button is used to sprint."), fpc.sprintButton);
        ...
        fpc.crouchButton = EditorGUILayout.TextField(new GUIContent("Crouch Button", "Determines what button is used to crouch."), fpc.crouchButton);
